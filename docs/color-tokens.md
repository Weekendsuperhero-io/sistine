...
| `--gradient` | `oklch(base + 63.53°) → oklch(base)` at `--gradient-l`/`--gradient-c` (knobs: `0.6 0.15`) | knobs: `0.68 0.14` | **Theme-aware** brand gradient in the foreground hue (`--glass-fg-h`), hue offset 3 ramp-steps out (360/17 ≈ 63.53°). Axis/direction variants are computed in JS (`lib/oklch-utils.ts` — see the `/components` demo); drives `glass-gradient` + the `gradient` Button. Twins: `--gradient-text` (full-opacity, for `GradientText`) and `--gradient-text-contrast` (`--gradient-contrast-l/-c` knobs: dark-on-light in light mode, light-on-dark in dark) |
