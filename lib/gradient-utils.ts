...
/**
 * Convert an sRGB (0-255) colour to an oklch() CSS string with alpha.
 */
function rgbToOklch(r: number, g: number, b: number, a: number): string {
  ...
  const aOk = 1.9779984951 * l1 - 2.428592205 * m1 + 0.4505937099 * s1;
  ...
  if (H < 0) H += 360;
  return `oklch(${L.toFixed(2)} ${C.toFixed(2)} ${H.toFixed(0)} / ${a})`;
}
...
export function gradientToCSS(gradient: Gradient): string {
  const colorStops = gradient.colors
    .map((color, index) => {
      ...
      return `${rgbToOklch(color.r, color.g, color.b, 0.8)} ${stop}%`;
    })
    .join(", ");
